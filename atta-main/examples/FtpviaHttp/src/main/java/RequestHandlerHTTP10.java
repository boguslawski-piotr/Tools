 ...
 * @author vorburger
 */
public abstract class RequestHandlerHTTP10 extends RequestHandler {
	...
	protected void handle(Socket socket) throws Exception {
		...
		response.setHeader(HTTPResponse.Header.Server, FtpviaHttp.getProgramName() + "/" + FtpviaHttp.getVersion());
		...
		if ( HTTPRequest.Version.HTTP11.toString().equals(request.getHTTPVersion()) ) {
			...
			throw new Exception("Don't know how to answer HTTP requests with this version header: " +  request.getHTTPVersion());
		}
	...
	private HTTPRequest getHTTPRequest(Socket socket) throws Exception {
		...
		if ( httpRequestLine == null ) {
			throw new Exception("No (or not enough) data received (within timeout)");
		}
		...
		try {
			...
			throw new Exception("HTTP Request Line (1st line) invalid, should be 'VERB URI VERSION' and not '" + httpRequestLine + "'; see RFC 2616, Section 5", ex);
		}
